...
function abrirConteoRapido(idx) {
  ...
  const lotesProducto = (db.lotes || []).filter(l => l.code === p.code);
  const hoy = new Date();
  const lotesHtml = lotesProducto.length ? `
    <div style="text-align:left;margin-bottom:14px;max-height:180px;overflow-y:auto;border:1px solid var(--border);border-radius:12px;padding:8px 10px">
      <div style="font-size:9px;font-weight:800;color:var(--muted);text-transform:uppercase;letter-spacing:.5px;margin-bottom:6px">Lotes conocidos (${lotesProducto.length}) · corrige si es necesario</div>
      ${lotesProducto.map(l => {
        const vencido = l.vencimiento && new Date(l.vencimiento) < hoy;
        return `<div style="display:flex;align-items:center;gap:8px;padding:6px 0;border-bottom:1px solid var(--border)">
          <div style="flex:1;min-width:0">
            <div style="font-size:11px;font-weight:700;color:var(--text)">Lote ${l.lote}</div>
            ${l.vencimiento ? `<div style="font-size:9px;color:${vencido ? 'var(--danger)' : 'var(--muted)'}">Vence ${l.vencimiento}${vencido ? ' · vencido' : ''}</div>` : ''}
          </div>
          <input type="number" min="0" value="${l.qty}" data-lote-id="${l.id}" class="lote-qty-input"
            style="width:64px;padding:6px 3px;border:1.5px solid var(--border);border-radius:8px;font-size:12px;font-weight:700;font-family:'Inter',sans-serif;text-align:center"
            oninput="actualizarSumaLotes(${idx})">
        </div>`;
      }).join('')}
    </div>` : '';
  ...
  modal.innerHTML = `
    <div style="background:var(--surface);border-radius:18px;padding:22px;max-width:340px;width:100%;text-align:center;max-height:90vh;overflow-y:auto">
      ...
      <div style="display:flex;gap:10px;margin-bottom:14px">
        ...
        <div style="flex:1;background:rgba(200,16,46,0.05);border:1.5px solid var(--danger);border-radius:12px;padding:10px">
          <div style="font-size:9px;font-weight:800;color:var(--danger);text-transform:uppercase;letter-spacing:.5px;margin-bottom:4px">Conteo físico${lotesProducto.length ? ' (suma lotes)' : ''}</div>
          <input id="conteo-rapido-input" type="number" min="0" autofocus placeholder="0" ${lotesProducto.length ? 'readonly' : ''}
...
function actualizarSumaLotes(idx) {
  const inputs = document.querySelectorAll('.lote-qty-input');
  let total = 0;
  inputs.forEach(inp => { total += parseFloat(inp.value) || 0; });
  const el = document.getElementById('conteo-rapido-input');
...
function confirmarConteoRapido(idx) {
  ...
  // Si el producto tiene lotes conocidos, guardar las cantidades corregidas de cada uno
  const loteInputs = document.querySelectorAll('.lote-qty-input');
  if (loteInputs.length) {
    loteInputs.forEach(inp => {
      const lote = (db.lotes || []).find(l => l.id === inp.dataset.loteId);
      if (lote) lote.qty = parseFloat(inp.value) || 0;
    });
    save();
  }
